...
enum NavState {
  ...
  LoginResumedAfterBackButton,

  LoginPausedByHomeButton,
  LoginResumedAfterHomeButton,

  UserLoggedOut,
  ResumedAfterUserLoggedOut,
  PausedWhileUserLoggedOut,

  LoginAgain
}

class LoginRequired<T extends UserBase> extends StatefulWidget {
  static late dynamic currentUser;
...
class _LoginRequiredState<T extends UserBase> extends State<LoginRequired<T>>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    switch (state) {
      ...
      case AppLifecycleState.inactive:
        // PauseLoginBack already taken care of
        // PauseLogout
        if (LoginRequired.navState == NavState.UserLoggedOut) {
          LoginRequired.navState = NavState.PausedWhileUserLoggedOut;
        }
        // PauseLoginHome
        // PauseNav
        else if (LoginRequired.navState != NavState.LoginPausedByBackButton) {
      ...
      case AppLifecycleState.resumed:
        switch (LoginRequired.navState) {
          case NavState.LoginPausedByHomeButton:
            LoginRequired.navState = NavState.LoginResumedAfterHomeButton;
            _printNavState();
            break;
          case NavState.LoginPausedByBackButton:
            setState(() {
              LoginRequired.navState = NavState.LoginResumedAfterBackButton;
              _printNavState();
            });
            break;
          case NavState.NormalPaused:
            ...
            _printNavState();
            break;
          case NavState.PausedWhileUserLoggedOut:
            setState(() {
              LoginRequired.navState = NavState.ResumedAfterUserLoggedOut;
              _printNavState();
            });
            break;
          default:
  ...
  void syncCurrentUser(Duration d) async {
    ...
    switch (LoginRequired.navState) {
      case NavState.Startup:
        if (FirebaseAuth.instance.currentUser == null) {
          ...
          WidgetsBinding.instance!.addPostFrameCallback(syncCurrentUser);
        }
        break;
      case NavState.LoginResumedAfterBackButton:
        assert(Platform.isAndroid);
        assert(LoginRequired.currentUser == null);
        // user needs to login
        _loginFlow();
        break;
      case NavState.ResumedAfterUserLoggedOut:
        assert(LoginRequired.currentUser == null);
        _loginFlow();
        break;
      case NavState.UserLoggedOut:
        assert(LoginRequired.currentUser == null);
        LoginRequired.navState = NavState.LoginAgain;
        _printNavState();
        _loginFlow();
        break;
      ...
      case NavState.NormalPaused:
      case NavState.PausedWhileUserLoggedOut:
      case NavState.LoginResumedAfterHomeButton:
      case NavState.NormalResumed:
        throw Exception(
